...
@section('title', 'Adjustment Stock - ' . $warehouse->warehouse_name)
...
<div class="row">
  <div class="col-12">
    <div class="card">
      ...
      <div class="card-body">
        <form action="{{ route('branch-warehouse.adjust', $warehouse->id) }}" method="POST" id="adjustmentForm">
          @csrf
            ...
            <div class="col-md-6">
              ...
              <select class="form-select @error('adjustment_type') is-invalid @enderror" 
                ...
                @foreach($adjustmentTypes as $key => $label)
                  <option value="{{ $key }}" @selected(old('adjustment_type') === $key)>{{ $label }}</option>
                @endforeach
              </select>
              @error('adjustment_type')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
              <small class="text-muted" id="adjustmentTypeHelp">Pilih alasan adjustment stock</small>
            </div>
            <div class="col-md-6">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  itemSelect.addEventListener('change', function() {
    ...
    if (selectedOption.value) {
      ...
      // Update placeholder and max for reduce operations
  ...
  form.addEventListener('submit', function(e) {
    ...
    // Validate stock for REDUCE operations
    if (['REDUCE', 'WASTE', 'DAMAGE', 'RETURN_SUPPLIER'].includes(adjustmentType)) {
      if (quantity > currentStock) {
        e.preventDefault();
        alert(`Stock tidak mencukupi! Tersedia: ${currentStock.toFixed(3)}, Diminta: ${quantity.toFixed(3)}`);
        quantityInput.focus();
        return false;
      }
    }
...
function updateAdjustmentInfo() {
  ...
  if (adjustmentType === 'ADD') {
    ...
    adjustmentTypeHelp.textContent = 'Tambah stock (misalnya: koreksi, stock opname)';
    ...
    // REDUCE/WASTE/DAMAGE: Negative adjustment
    ...
    warningText.innerHTML = '<strong>Peringatan:</strong> Adjustment akan mengurangi stock dari balance bulan ini. Pastikan data sudah benar.';
