...
class LoginController extends Controller
{
    ...
    protected function authenticated(Request $request, $user)
    {
        ...
        switch ($user->role) {
            case 'admin':
                return redirect()->route('beranda')->with('login_role', 'admin');
            case 'manager':
                return redirect()->route('beranda')->with('login_role', 'manager');
            case 'staff':
                return redirect()->route('beranda')->with('login_role', 'staff');
            default:
    ...
    public function logout(Request $request)
    {
        ...
        return $request->wantsJson()
            ? new JsonResponse([], 204)
            : redirect('/login')->with('success', 'Anda telah berhasil logout.');
    }
    ...
    /**
     * Quick login for development/testing (remove in production)
     *
     * @param  \Illuminate\Http\Request  $request
    ...
    public function quickLogin(Request $request)
    {
        ...
        $email = $request->get('email', 'admin@chickingbjm.com');
        $user = User::where('email', $email)->first();

        if ($user) {
            Auth::login($user);
            return redirect()->route('beranda')->with('success', 'Quick login successful!');
        }

        return redirect()->route('login')->with('error', 'User not found for quick login.');
    }
    ...
    public function updateProfile(Request $request)
    {
        ...
        if ($request->filled('password')) {
            if (!$request->filled('current_password')) {
                return redirect()->back()
                    ->withErrors(['current_password' => 'Password saat ini harus diisi'])
                    ->withInput();
            }
